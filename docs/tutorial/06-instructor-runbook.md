...
This guide is for the instructors and staff running the 2-hour summer-school tutorial in France. It covers **pre-session prep**, **real-robot safety**, **timing cuts**, **device switching**, **talking points per tier**, and **answer keys for all exercises**.

---

## Solution Files Setup

The three exercise solution files are **gitignored and not in the public repo**. Drop them into `mile_franka/tutorial/` before running:

```
mile_franka/tutorial/_loss_solution.py
mile_franka/tutorial/_intervention_model_solution.py
mile_franka/tutorial/_scripted_intervener_solution.py
```

These files are base64-obfuscated (participants can't read them if they stumble on the instructor machine). The loaders detect them automatically — without them, participants must implement the exercises themselves with no fallback.
...
### Exercise 1 — Loss (`_loss_solution.py`)

- **Discrete loss**: NLL of the 2-class intervention head. Clamp `intervention_prob` to `[1e-7, 1-1e-7]`, take `log`, call `F.nll_loss(log_probs, ground_truth_intervention)`.
- **Continuous loss**: Gaussian NLL of `ground_truth_action` under `Normal(mu, exp(log_std))`, summed over action dims, averaged — computed **only on ν=1 rows**. Zero if no ν=1 rows.
- **Combined**: `loss = LAMBDA1 * continuous_loss + LAMBDA2 * discrete_loss`.

Common bugs:
- Forgot to clamp before log → NaN loss.
- Didn't mask to ν=1 rows for continuous loss → penalises non-intervention steps.
- Wrong weight order (LAMBDA1 on discrete, LAMBDA2 on continuous) → loss still trains but is off by convention.
...
### Exercise 2 — Intervention Model (`_intervention_model_solution.py`)

MILE's probit model: `p(ν=1|s) ≈ E_{a~π}[Φ(log π(a|s) − E_{â~π̂}[log π(â|s)] − cost)]`

Steps:
1. Sample 1000 actions from the mental model π̂ → `mental_model_samples`.
2. Compute their log-prob under the robot policy π → baseline `E[log π(â|s)]`.
3. Sample 1000 actions from π.
4. For each: `gap = log π(a|s) − baseline − cost`.
5. `CDF(gap)` via `Normal(0, cdf_scale)` → per-sample probability.
6. Mean across samples → scalar `p(ν=1|s)` per state.

Key insight: large gap (policy assigns high log-prob to its own actions but low log-prob to what the human expected) → high intervention probability.

What participants typically try:
- **Entropy of policy**: works, ignores mental model.
- **KL(mental_model || policy)**: good intuition, harder to implement in closed form.
- **Fixed rate (10%)**: valid baseline, shows MILE's model is doing more.

Reveal framing: *"Your rule is the ground truth. The computational model learns when your rule would fire — from distributions alone, with no privileged expert access."*
...
### Exercise 3 — Scripted Intervener (`_scripted_intervener_solution.py`)

Hard-coded binary rule:
- Translation drift: L2 norm of expert vs policy Cartesian delta > 0.5.
- Gripper disagreement: expert and policy disagree on open/close.

Threshold `0.5` is tuned for Franka action scale (~0–1 Cartesian deltas, ~2 cm/step). Gripper gets a hard trigger: wrong open/close at grasp/release causes task failure regardless of position error.
...
If participants try the cost ablation (doubling cost for MetaWorld): fewer interventions → less human signal → slower or no improvement. At extreme cost → p(ν=1|s) → 0 → degenerates to base policy.

---

### Lambda Ablation Expected Results

| λ₁ | λ₂ | Expected |
|---|---|---|
| 0.0 | 1.0 | Policy learns *when* humans step in but NOT the right action → no task improvement |
| 1.0 | 0.0 | Policy imitates intervention actions but mental model untrained → noisy rollout |
| 1.0 | 1.0 | Full MILE — baseline |
| 2.0 | 0.5 | Slightly better action imitation; usually comparable to baseline |

---

### Reveal Order (Suggested)

1. **Scripted intervener first** — simplest; pure heuristic. Ask: "what rule did you use?"
2. **Computational model second** — reveal the probit model and the connection: "the computational model learns what you hard-coded, without privileged access."
3. **Loss last** — most mechanical; confirm BCE + NLL by inspecting loss curves.
...
**Q: Should I tell participants the answer keys exist?**

A: No. They should implement all three exercises themselves. If they get stuck (>15 min on any one), give hints before revealing:
- **Loss**: "Check that ν=1 steps use Gaussian NLL, and ν=0 steps use only BCE. Are you masking correctly?"
- **Intervention model**: "What does it mean for the robot to need help? Can you compare the two distributions?"
- **Scripted intervener**: "What's the simplest signal that tells you the policy is going wrong?"

If they still fail, mention the answer key and let them review it during the debrief.
